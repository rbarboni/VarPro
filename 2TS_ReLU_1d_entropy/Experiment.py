...
## 1 Projection before training
print('Performing 1 projection step before training')

# projection by performing optimization on the outer layer
student.outer.weight = nn.Parameter(data=torch.ones_like(student.outer.weight), requires_grad=True)

outer_optimizer = torch.optim.SGD([student.outer.weight], lr=0.2*lr_outer)
outer_problem = LearningProblem(student, train_loader, outer_optimizer, criterion)
outer_problem.train(500, progress=False)
